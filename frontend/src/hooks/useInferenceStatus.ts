...
function mapRequestToStatus(request: BackendInferenceRequest, coverageRecommendation?: string): DemoStatus {
  ...
  const leaderResult =
    metadata.text_leader_result && typeof metadata.text_leader_result === 'object'
      ? (metadata.text_leader_result as Record<string, unknown>)
      : {}
  const agentCommitmentTx =
    typeof metadata.agent_commitment_tx === 'string' ? metadata.agent_commitment_tx : undefined
  const receiptRoot = typeof leaderResult.receipt_root === 'string' ? leaderResult.receipt_root : undefined
  const receiptsCid = typeof leaderResult.receipts_cid === 'string' ? leaderResult.receipts_cid : undefined
  const outputHash = typeof leaderResult.output_hash === 'string' ? leaderResult.output_hash : undefined
  const traceHash = typeof leaderResult.trace_hash === 'string' ? leaderResult.trace_hash : undefined
  ...
  return {
    ...
    text_result:
      request.mode === 'text' || request.text_mode
        ...
        : undefined,
    uavp:
      receiptRoot || receiptsCid || traceHash || agentCommitmentTx
        ? {
            agent_commitment_tx: agentCommitmentTx,
            receipt_root: receiptRoot,
            receipts_cid: receiptsCid,
            output_hash: outputHash,
            trace_hash: traceHash,
...
function mapTextStatusToDemoStatus(
  ...
  return {
    ...
    text_result: {
      output_cid: request.output_cid ?? undefined,
      commitment_hash: request.commitment_hash ?? undefined,
      receipt_root: request.receipt_root ?? undefined,
      receipts_cid: request.receipts_cid ?? undefined,
      output_hash: request.output_hash ?? undefined,
      trace_hash: request.trace_hash ?? undefined,
      encrypted_output_key_high: request.encrypted_output_key_high ?? undefined,
      encrypted_output_key_low: request.encrypted_output_key_low ?? undefined,
    },
    uavp:
      request.receipt_root || request.receipts_cid || request.trace_hash
        ? {
            receipt_root: request.receipt_root ?? undefined,
            receipts_cid: request.receipts_cid ?? undefined,
            output_hash: request.output_hash ?? undefined,
